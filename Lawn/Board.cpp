...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	if (aNormalPlant && aNormalPlant->IsUpgradableTo(aPlantingSeedType))
	{
		...
		if (aNormalPlant->mSeedType == SEED_CACTUS && aPlantingSeedType == SeedType::SEED_FIREPEA) {
			aPlantingSeedType = SEED_FIRECACTUS;
		}
		aNormalPlant->Die();
	}
	...
	else if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK)
	{
		...
		if (mCursorObject->mType == SEED_FIREPEA && aPlantingSeedType == SEED_FIRECACTUS) {
			toPlant = SEED_FIRECACTUS;
		}
