...
PlantDefinition gPlantDefs[SeedType::NUM_SEED_TYPES] = {  
    { SeedType::SEED_PEASHOOTER,        nullptr, ReanimationType::REANIM_PEASHOOTER,    0,  100,    750,    PlantSubClass::SUBCLASS_SHOOTER,    100,    _S("PEASHOOTER") },
    { SeedType::SEED_SUNFLOWER,         nullptr, ReanimationType::REANIM_SUNFLOWER,     1,  50,     750,    PlantSubClass::SUBCLASS_NORMAL,     1800,   _S("SUNFLOWER") },
    { SeedType::SEED_CHERRYBOMB,        nullptr, ReanimationType::REANIM_CHERRYBOMB,    3,  125,    3000,   PlantSubClass::SUBCLASS_NORMAL,     0,      _S("CHERRY_BOMB") },
    ...
    { SeedType::SEED_HYPNOSHROOM,       nullptr, ReanimationType::REANIM_HYPNOSHROOM,   10, 50,     3000,   PlantSubClass::SUBCLASS_NORMAL,     0,      _S("HYPNO_SHROOM") },
    ...
    { SeedType::SEED_PEADRONE,        nullptr, ReanimationType::REANIM_PEADRONE,    0,  100,    750,    PlantSubClass::SUBCLASS_SHOOTER,    100,    _S("PEADRONE") },
...
{ SeedType::SEED_GATLINGPEA,        nullptr, ReanimationType::REANIM_GATLINGPEA,         5,  300,    5000,   PlantSubClass::SUBCLASS_SHOOTER,    150,    _S("GATLING_PEA") },
    ...
    { SeedType::SEED_GRAVEBUSTER,       nullptr, ReanimationType::REANIM_GRAVE_BUSTER,  40, 125,     3000,    PlantSubClass::SUBCLASS_NORMAL,     0,      _S("GRAVE_BUSTER") },
...
bool Plant::FindTargetAndFire(int theRow, PlantWeapon thePlantWeapon)
{
    ...
    else if (aHeadReanim && aHeadReanim->TrackExists("anim_shooting"))
    {
        ...
        mShootingCounter = 33;
        if (mSeedType == SeedType::SEED_REPEATER || mSeedType == SeedType::SEED_FIREPEA || mSeedType == SeedType::SEED_SPLITPEA || mSeedType == SeedType::SEED_LEFTPEATER)
        {
...
void Plant::LaunchQuatroThreepeater()
{
    int rowAbove = mRow - 1;
    int rowBelow = mRow + 1;
...
bool Plant::MakesSun()
{
    return mSeedType == SeedType::SEED_SUNFLOWER || mSeedType == SeedType::SEED_TWINSUNFLOWER || mSeedType == SeedType::SEED_QUATROSUNFLOWER || mSeedType == SeedType::SEED_SUNSHROOM;
}
...
void Plant::UpdateProductionPlant()
{
    ...
    if (mLaunchCounter <= 0)
    {
        ...
        else if (mSeedType == SeedType::SEED_QUATROSUNFLOWER)
        {
            mBoard->AddCoin(mX, mY, CoinType::COIN_SUN, CoinMotion::COIN_MOTION_FROM_PLANT);
            mBoard->AddCoin(mX, mY, CoinType::COIN_SUN, CoinMotion::COIN_MOTION_FROM_PLANT);
            mBoard->AddCoin(mX, mY, CoinType::COIN_SUN, CoinMotion::COIN_MOTION_FROM_PLANT);
...
void Plant::UpdateSpikeweed()
{
    ...
    {
        uint DamageFlags = 2U;
        SetBit(DamageFlags, (int)DamageFlags::DAMAGE_MOOS, true);
...
Reanimation* Plant::AttachBlinkAnim(Reanimation* theReanimBody)
{
    ...
    else if (mSeedType == SeedType::SEED_THREEPEATER || mSeedType == SeedType::SEED_QUATROTHREEPEATER)
    {
        int aHit = Rand(3);
        if (aHit == 0)
        {
    ...
    else if (mSeedType == SeedType::SEED_PEASHOOTER || mSeedType == SeedType::SEED_REPEATER || mSeedType == SeedType::SEED_SNOWPEA || mSeedType == SeedType::SEED_FIREPEA || mSeedType == SeedType::SEED_LEFTPEATER || mSeedType == SeedType::SEED_GATLINGPEA)
    {
...
void Plant::UpdateShooting()
{
    ...
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    Reanimation* aHeadReanim = mApp->ReanimationTryToGet(mHeadReanimID);
    if (mSeedType == SeedType::SEED_THREEPEATER || mSeedType == SeedType::SEED_QUATROTHREEPEATER)
    {
        Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
        Reanimation* aHeadReanim3 = mApp->ReanimationGet(mHeadReanimID3);
...
void Plant::DrawSeedType(Graphics* g, SeedType theSeedType, SeedType theImitaterType, DrawVariation theDrawVariation, float thePosX, float thePosY)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else if (aSeedType == SeedType::SEED_TWINSUNFLOWER || aSeedType == SeedType::SEED_QUATROSUNFLOWER)
            {
                aCelRow = 1;
            }
...
void Plant::Fire(Zombie* theTargetZombie, int theRow, PlantWeapon thePlantWeapon)
{
    if (mSeedType == SeedType::SEED_FUMESHROOM)
    {
        ...
        DoRowAreaDamage(20, fumeDamageFlags);
    ...
    case SeedType::SEED_CABBAGEPULT:
        ...
        break;
    case SeedType::SEED_PUFFSHROOMPULT:
        aProjectileType = ProjectileType::PROJECTILE_PUFFPULT;
        break;
    case SeedType::SEED_KERNELPULT:
    ...
    else if (mSeedType == SeedType::SEED_THREEPEATER || mSeedType == SeedType::SEED_REPEATER)
    {
        aOriginX = mX + 45;
        aOriginY = mY + 10;
    }
...
Zombie* Plant::FindTargetZombie(int theRow, PlantWeapon thePlantWeapon)
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        ...
        {
            if (mSeedType == SeedType::SEED_REPEATER || mSeedType == SeedType::SEED_PEASHOOTER || mSeedType == SeedType::SEED_CACTUS || mSeedType == SeedType::SEED_FIREPEA)
            {
...
void Plant::Die()
{
    ...
    if (IsOnBoard() && mSeedType == SeedType::SEED_WALLNUT)
    {
        int aPosX = mX + mWidth / 2;
        int aPosY = mY + mHeight / 2;
        int aDamageRangeFlags = GetDamageRangeFlags(PlantWeapon::WEAPON_PRIMARY);
