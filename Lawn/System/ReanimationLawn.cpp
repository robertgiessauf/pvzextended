...
MemoryImage* ReanimatorCache::MakeCachedPlantFrame(SeedType theSeedType, DrawVariation theDrawVariation)
{
	...
	else
	{
		...
		if (theSeedType == SeedType::SEED_PEASHOOTER || theSeedType == SeedType::SEED_SNOWPEA || theSeedType == SeedType::SEED_FIREPEA || theSeedType == SeedType::SEED_REPEATER ||
			theSeedType == SeedType::SEED_LEFTPEATER || theSeedType == SeedType::SEED_GATLINGPEA || theSeedType == SeedType::SEED_GATLINSNOWGPEA)
		{
		...
		else if (theSeedType == SeedType::SEED_THREEPEATER || theSeedType == SeedType::SEED_QUATROTHREEPEATER)
		{
...
MemoryImage* ReanimatorCache::MakeCachedZombieFrame(ZombieType theZombieType)
{
	...
	if (aZombieDef.mReanimationType == ReanimationType::REANIM_ZOMBIE)
	{
		...
		else if (theZombieType == ZombieType::ZOMBIE_PEA_THREEPEATER)
		{
			Reanimation aReanimHead;
			aReanimHead.ReanimationInitializeType(aPosX + 72, aPosY - 2, ReanimationType::REANIM_THREEPEATER);
			aReanimHead.SetFramesForLayer("anim_head_idle");
			aReanimHead.OverrideScale(-1.0f, 1.0f);
			aReanimHead.Draw(&aMemoryGraphics);
			aReanim.AssignRenderGroupToTrack("Zombie_neck", RENDER_GROUP_HIDDEN);
